...
var subUnits = document.querySelector(".sub-units");
...
colors.addEventListener("click", function (e) {
  ...
  e.target.className.split(' ').forEach(function (color) {
    if (color === 'red') _color = 'red';else if (color === 'black') _color = 'black';else if (color === 'green') _color = 'green';else {
      console.log('color出错');
    }
  });
  console.log(_color);
...
sizes.addEventListener("click", function (e) {
  if (e.target.className.search('line-1') != -1) {
    ctx.lineWidth = 1;
  } else if (e.target.className.search('line-3') != -1) ctx.lineWidth = 3;else if (e.target.className.search('line-5') != -1) ctx.lineWidth = 5;else {
    console.log('lineWidth出错', e.target.className);
  }
});
subUnits.addEventListener("click", function subUnitsClick(e) {
  var subUnit = e.target.getAttribute("name");
  ...
  switch (subUnit) {
    ...
    case "download":
      var fileName = prompt('请输入文件名', 'filename.png');
      if (!fileName) return;
